...
module.exports = {
	...
	run: async (client, message, args, embed) => {
		...
		if (!member) return message.channel.send(embed.setDescription("Bir kullanıcı belirtmelisin! @Deasn/ID"));
		...
		if (args[0] === "ekle" || args[0] === "add") {
			...
			if (coinData && client.ranks.some(x => coinData.coin >= x.coin && !member.hasRole(x.role))) {
				...
				message.guild.channels.cache.get(conf.rankLog).send(embed.setDescription(`${member.toString()} üyesine ${message.member.toString()} tarafından **${count}** adet coin eklendi ve kişiye ${roles.filter(x => roles.indexOf(x) === roles.length-1).map(x => Array.isArray(x.role) ? x.role.map((r) => `<@&${r}>`).join(", ") : `<@&${x.role}>`).join("\n")} rolleri verildi!`));
			}
